...
DEFAULT_SYSTEM_PROMPT = """You write a weekly review for one family
caregiver looking after someone living with dementia at home. They are
tired. They will read this with their coffee, once.
...
- Say plainly that the demo household is simulated.
- Write four sentences or fewer, in plain warm language, second person.
  No emojis. No dashes of any kind as punctuation, and write date ranges
  as "September 24 to 30".
...
# No local tools on purpose. This agent's only capabilities are the five
# tools the MCP server exposes, so it cannot reach the household by any
# path Alexa+ could not also take.
