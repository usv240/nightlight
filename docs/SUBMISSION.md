...
**And what that restraint cost, on the same data: of 34 resident-labeled night-time exits, Nightlight flagged 7 as unusual. Twenty-six of the 27 it did not flag were exits the resident returned from within thirty minutes, by the corpus's own label, and the misses concentrate in households where the door already opens thirty times a night.** A reduction figure alone is worth little, because waking someone less often is trivially achievable by doing nothing. Both numbers, the per-home table and the reasons not to over-read eight homes are in [EVAL.md](EVAL.md).
